...
The plots below depict the Kaplan-Meier curves (the risk of death over time) of variables we found to demonstrate clear effects that we could draw inference from. The plots also include the p-value dervied from conducting log-rank tests on the Kaplan-Meier curves.
</br>
</br>
...
ggsave("output/cox_graph_complete_case.png", ggforest(fit.coxph, data=cardio_new), device = "png", width = 7, height = 7, units = "in")
```
