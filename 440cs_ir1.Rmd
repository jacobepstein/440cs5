...
fit_age=survfit(Surv(DAYS2LKA, DEATH) ~ age_low,data=cardio_new)
ggsurvplot(fit_age,data=cardio_new,pval=TRUE)

#race (white v. non-white)
fit_white=survfit(Surv(DAYS2LKA, DEATH) ~ RACE_G, data=cardio_new)
ggsurvplot(fit_white,data=cardio_new,pval=TRUE)

#note: can explore more variables when code is debugged (CHFSEV could be good... groups: {CHFSV=0, CHFSV!=0})

ggsurvplot(survfit(Surv(DAYS2LKA, DEATH) ~ CHFSEV, data=cardio_new), data=cardio_new, pval=TRUE)
...
[TODO: elaborate on age, U-shaped trend, not simply increasing],

[TODO: use full names, not variable names, and give context to why the relationship do or don't make sense]
