...
In order to address the issues associated with missing data, we built a complete case, and we estimated survival functions using both Cox proportional hazards regression modelling and Kaplan-Meier curves.
...
In order to run the Cox model, we established that our survival object would measure time with DAYS2LKA as we are working with censored, not interval, data, and we set the event to be 0 if the individual died and 1 if the individual stayed alive. We modelled the survival object using the following covariates: YRCATH_G, AGE_G, GENDER, RACE_G, HXSMOKE, HXCEREB, CHFSEV, HXCOPD, HXDIAB, HXHYL, S3, LMST, NUMDZV.
</br>
</br>
Our forest plot indicates that as an individual's age increases, so does this person's risk of dying. Furthermore, the same trend occurs for white and non-white individuals, where non-white individuals are at higher risk of dying. Moreover, this trend appears in the variable CHFSEV as well, where individuals with a severity of 0 are the least at risk. Interestingly, YRCATH_G has decreasing risk as the group number moves from 1-3, then increases for groups 4-7.

```{r}
#Kaplan-Meier curves

surv_ob=Surv(time=cardio$DAYS2LKA,event=cardio$DEATH)

#gender
fit_gender=survfit(surv_ob~GENDER,data=cardio)
ggsurvplot(fit_gender,data=cardio,pval=TRUE)

#age (low is <40)
cardio=cardio%>%mutate(age_low=case_when(AGE_G%in%seq(1,4,1)~1,AGE_G%in%seq(4,13,1)~0))
fit_age=survfit(surv_ob~age_low,data=cardio)
ggsurvplot(fit_age,data=cardio,pval=TRUE)

#race (white v. non-white)
cardio=cardio%>%mutate(white=case_when(RACE_G==1~1,RACE_G%in%c(2,3)~0))
fit_white=survfit(surv_ob~white,data=cardio)
ggsurvplot(fit_white,data=cardio,pval=TRUE)

#note: can explore more variables when code is debugged (CHFSEV could be good... groups: {CHFSV=0, CHFSV!=0})
```
